...
export default function Heatmap() {
  ...
  const filtered = useMemo(() => {
    ...
    return items;
  }, [tickers, category, sortBy]);

  const summary = useMemo(() => {
    if (!tickers.length) return null;
    const usdtPairs = tickers;
    const gainers = usdtPairs.filter(t => t.change > 0).length;
    const losers = usdtPairs.filter(t => t.change < 0).length;
    const sorted = [...usdtPairs].sort((a, b) => b.change - a.change);
    const topGainer = sorted[0];
    const topLoser = sorted[sorted.length - 1];
    const totalVol = usdtPairs.reduce((s, t) => s + t.volume, 0);
  ...
  const blockSizes = useMemo(() => {
    ...
    const maxVol = Math.max(...filtered.map(t => t.volume));
    return filtered.map(t => {
      const ratio = t.volume / maxVol;
    ...
    return (
      <div style={{ background: bg, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ color: textSecondary, fontSize: 14 }}>Loading market data...</div>
        </div>
      </div>
    );
  }
        ...
        <div style={{ textAlign: 'center', color: '#ef4444' }}>
          ...
          <button
            ...
            style={{
              ...
              padding: '8px 20px', cursor: 'pointer', fontSize: 13, fontWeight: 500,
            }}
          >
            Retry
          </button>
        ...
        <div ref={sortRef} style={{ position: 'relative' }}>
          <button
            ...
            style={{
              ...
              background: cardBg, color: textPrimary, fontSize: 12, cursor: 'pointer',
            }}
          >
            <ArrowUpDown size={13} />
          ...
          {sortMenuOpen && (
            <div
              style={{
                ...
                padding: 4, zIndex: 50, minWidth: 120,
              ...
              {SORT_OPTIONS.map(opt => (
                <button
                  ...
                  style={{
                    ...
                    background: sortBy === opt.key ? (dark ? '#3b3f51' : '#e2e8f0') : 'transparent',
          ...
          return (
            <div
              ...
              style={{
                ...
                position: 'relative',
                overflow: 'hidden',
                boxShadow: hoveredTicker?.symbol === ticker.symbol
                  ? '0 0 0 2px #3b82f6, 0 4px 12px rgba(59,130,246,0.3)'
                  : 'none',
                transform: hoveredTicker?.symbol === ticker.symbol ? 'scale(1.04)' : 'scale(1)',
                zIndex: hoveredTicker?.symbol === ticker.symbol ? 10 : 1,
              }}
            >
              <div style={{
                color: colors.text, fontSize: w >= 100 ? 13 : 11,
                fontWeight: 700, lineHeight: 1.1, letterSpacing: 0.3,
              }}>
                {ticker.name}
              </div>
              <div style={{
                color: colors.text, fontSize: w >= 100 ? 12 : 10,
                fontWeight: 600, opacity: 0.95, marginTop: 1,
              }}>
                {ticker.change >= 0 ? '+' : ''}{ticker.change.toFixed(2)}%
              </div>
              {w >= 100 && (
                <div style={{
                  color: colors.text, fontSize: 9, opacity: 0.7, marginTop: 1,
                }}>
      ...
      {hoveredTicker && (
        <div
          style={{
            ...
            border: `1px solid ${dark ? '#3b3f51' : '#d1d5db'}`,
            borderRadius: 8,
            padding: '10px 14px',
            zIndex: 1000,
            pointerEvents: 'none',
            boxShadow: dark ? '0 8px 24px rgba(0,0,0,0.6)' : '0 8px 24px rgba(0,0,0,0.15)',
            minWidth: 180,
          }}
        >
          <div style={{ fontWeight: 700, fontSize: 14, color: textPrimary, marginBottom: 6 }}>
            {hoveredTicker.name}
            <span style={{ fontWeight: 400, fontSize: 11, color: textSecondary, marginLeft: 6 }}>/ USDT</span>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '3px 12px', fontSize: 12 }}>
            ...
            <span style={{
              color: hoveredTicker.change >= 0 ? '#22c55e' : '#ef4444',
              fontWeight: 600, textAlign: 'right',
            }}>
              {hoveredTicker.change >= 0 ? '+' : ''}{hoveredTicker.change.toFixed(2)}%
            </span>
            <span style={{ color: textSecondary }}>24h Volume</span>
